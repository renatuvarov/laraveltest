...
@section('body')
    <div class="container text-center">
        <h2 class="h2 mb-5 display-4">Обновить книгу</h2>
        <form class="text-left w-50 m-auto" method="post" action="{{ route('books.update', ['book' => $book->id]) }}"
              enctype="multipart/form-data">
            @csrf
            @method('put')
            <div class="form-group required">
                <input value="{{ old('title', $book->title) }}" type="text" class="form-control @error('title') is-invalid @enderror"
                    ...
                    <select class="form-control" id="author" name="author_id">
                        @foreach($authors as $author)
                            <option value="{{ $author->id }}"
                                    @if((old('author_id', $book->author->id ?? null) == $author->id)) selected @endif>{{ $author->name }}</option>
                        @endforeach
                    </select>
                    @error('author_id')
                    <small class="form-text text-muted ml-2" style="color: #c82333 !important;">{{$message}}</small>
